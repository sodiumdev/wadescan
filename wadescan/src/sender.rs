...
use crate::range::StaticScanRanges;
...
impl<'a> PacketSender<'a> {
    ...
    #[inline]
    pub fn frames(kind: SenderKind, gateway_mac: &[u8; 6], interface_mac: &[u8; 6], source_ip: Ipv4Addr, umem: &Umem, ping_data: &'static [u8]) -> Frames<'a> {
        Frames {
            syn_frames: Self::syn_frames(&kind, gateway_mac, interface_mac, &source_ip, umem),
            ack_frames: Self::ack_frames(&kind, gateway_mac, interface_mac, &source_ip, umem),
            psh_frames: Self::psh_frames(&kind, gateway_mac, interface_mac, &source_ip, umem, ping_data),
            fin_frames: Self::fin_frames(&kind, gateway_mac, interface_mac, &source_ip, umem),
    ...
    #[inline]
    fn syn_frames(kind: &SenderKind, gateway_mac: &[u8; 6], interface_mac: &[u8; 6], source_ip: &Ipv4Addr, umem: &Umem) -> Vec<Frame<'a>> {
    ...
    #[inline]
    fn ack_frames(kind: &SenderKind, gateway_mac: &[u8; 6], interface_mac: &[u8; 6], source_ip: &Ipv4Addr, umem: &Umem) -> Vec<Frame<'a>> {
        let offset = match kind {
            SenderKind::Scanner => 0,
            SenderKind::Responder => umem.len_frames() / 4,
        };
        ...
        let frames = umem.len_frames() / 8;
        ((frames + offset)..(frames * 2 + offset)).map(|n| {
            let mut frame = umem.frame(BufIdx(n)).unwrap();
            let base = unsafe { frame.addr.as_mut() };
    ...
    #[inline]
    fn psh_frames(kind: &SenderKind, gateway_mac: &[u8; 6], interface_mac: &[u8; 6], source_ip: &Ipv4Addr, umem: &Umem, ping_data: &'static [u8]) -> Vec<Frame<'a>> {
        let offset = match kind {
            SenderKind::Scanner => 0,
            SenderKind::Responder => umem.len_frames() / 4,
        };
        ...
        let frames = umem.len_frames() / 8;
        ((frames * 2 + offset)..(frames * 3 + offset)).map(|n| {
            let mut frame = umem.frame(BufIdx(n)).unwrap();
            let base = unsafe { frame.addr.as_mut() };
    ...
    #[inline]
    fn fin_frames(kind: &SenderKind, gateway_mac: &[u8; 6], interface_mac: &[u8; 6], source_ip: &Ipv4Addr, umem: &Umem) -> Vec<Frame<'a>> {
        let offset = match kind {
            SenderKind::Scanner => 0,
            SenderKind::Responder => umem.len_frames() / 4,
        };
        ...
        let frames = umem.len_frames() / 8;
        ((frames * 3 + offset)..(frames * 4 + offset)).map(|n| {
            let mut frame = umem.frame(BufIdx(n)).unwrap();
            let base = unsafe { frame.addr.as_mut() };
    ...
    pub fn send_syn(&mut self, ip: &Ipv4Addr, port: u16, seed: u64) {
        ...
        let (addr, syn_frame) = unsafe {
            &mut *self.frames.syn_frames.as_mut_ptr().add(self.syn_frame)
        };
        ...
        // checksum is not skipped while calculating!!! TODO: fix without any performance drops
        {
            ...
            syn_frame[50..52].copy_from_slice(
        ...
        {
            let checksum = checksum::ipv4(
                &syn_frame[14..34]
            );

            syn_frame[24..26].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
    ...
    pub fn send_ack(&mut self, ip: &Ipv4Addr, port: u16, seq: u32, ack: u32) {
        ...
        // SAFETY: bound checks are done above
        let (addr, ack_frame) = unsafe {
            &mut *self.frames.ack_frames.as_mut_ptr().add(self.ack_frame)
        };
        ...
        // checksum is not skipped while calculating!!! TODO: fix without any performance drops
        {
            ...
            ack_frame[50..52].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
        ...
        // no need to set the checksum to 0 here because it's skipped while calculating
        {
            ...
            ack_frame[24..26].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
    ...
    pub fn send_psh(&mut self, ip: &Ipv4Addr, port: u16, seq: u32, ack: u32) {
        ...
        // SAFETY: bound checks are done above
        let (addr, psh_frame) = unsafe {
            &mut *self.frames.psh_frames.as_mut_ptr().add(self.psh_frame)
        };
        ...
        // checksum is not skipped while calculating!!! TODO: fix without any performance drops
        {
            ...
            psh_frame[50..52].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
        ...
        // no need to set the checksum to 0 here because it's skipped while calculating
        {
            ...
            psh_frame[24..26].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
    ...
    pub fn send_fin(&mut self, ip: &Ipv4Addr, port: u16, seq: u32, ack: u32) {
        ...
        // SAFETY: bound checks are done above
        let (addr, fin_frame) = unsafe {
            &mut *self.frames.fin_frames.as_mut_ptr().add(self.fin_frame)
        };
        ...
        // checksum is not skipped while calculating!!! TODO: fix without any performance drops
        {
            ...
            fin_frame[50..52].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
        ...
        // no need to set the checksum to 0 here because it's skipped while calculating
        {
            ...
            fin_frame[24..26].copy_from_slice(
                &checksum.to_be_bytes()
            );
        }
